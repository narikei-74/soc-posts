<x-layout>
    <x-slot name="title">
        S.O.C Posts - キャンパー 掲示板
    </x-slot>
    <x-slot name="link">
        <link rel="stylesheet" href="{{ url('css/style.css') }}">
        <link rel="stylesheet" href="{{ url('css/post.css') }}">
        <link rel="stylesheet" href="{{ url('css/camp.css') }}">
    </x-slot>

    <div class="main">
        <div class="back-link">
            <a href="{{ route('index_view') }}">home</a> &raquo; <span>キャンパーの知恵広場</span>
        </div>
        ...
        <form class="search" action="{{ route('camp_search') }}" method="get">
            @csrf
            ...
            <br>
            <br>
            <a href="{{ route('camp_view') }}">検索結果をクリア</a>
        </form>

        <form class="do_post" action="{{ route('camp_post') }}" method="POST">
            @csrf
            <h2>投稿する</h2>
